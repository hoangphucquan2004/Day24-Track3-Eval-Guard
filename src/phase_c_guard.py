...
async def check_input_rail(text: str, rails=None) -> dict:
    ...
    # NeMo có thể trả về str hoặc dict {"role": ..., "content": ...}
    if isinstance(response, dict):
        response_text = response.get("content", "") or str(response)
    else:
...
if __name__ == "__main__":
    ...
    # Task 12: P95 latency (warm Presidio — skip first cold run)
    analyzer, anonymizer = setup_presidio()
    rails = setup_nemo_rails()
    ...
    guard_report = {
        ...
        "adversarial_suite": {
            ...
            "pass_rate": round(passed / len(adv_results), 3) if adv_results else 0,
            "results": adv_results,
        },
        "latency": latency,
    }
